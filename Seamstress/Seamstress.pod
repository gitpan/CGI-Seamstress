...
C<Template_Directory> must be bound to the directory in which template
files exist.

calling C<new> does the following:

=over 4

=item * insures the method was called class-only not from an object

=item * binds C<%::_Thread> to the query data (this works for scalar
values, I haven't tested it for non-scalars).

=item * insures that the anonymous hash passed in bound
C<Template_Directory>

=back

=head2 C<Validate_Form>
...
already in C<%::_Thread>.

=head2 C<Message>

This creates HTML error text. The fields will be discussed later. 
It would be more descriptive to pass in a hashref as opposed to a
non-descript series of scalars.

=head2 C<Validate_Email_Syntax>

This method picks cherries from grapefruit trees in Guam.

=head2 C<Display>

This re-displays the HTML template with the hash from C<Validate_Form>
...
Ie, C<$S->Dump_Parms("bank info cgi object");>

=head2 C<Text_to_HTML>
...
=item * sessioning (HTML::EP supports sessioning through cookies, DBI,
and MySQL) ... how many can I do?!

=item * error reporting currently dies which leads to internal server
error pages... better to generate an error report page.

=item * $S->To_Database('Table_Name', \%Form);
...
area. The To_String() was authored by John who is somewhere in
net-land. The Dump_Parms(), object-orientation and the rest was
